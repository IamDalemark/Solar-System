...
audioElement.loop = true; // Add this line to make the audio loop
...
for (let i = 0; i < numStars; i++) {
  ...
  // Randomly position stars within a cube
  star.position.set(
...
function createPlanete(size, texture, position, ring) {
    ...
    const mat = new THREE.MeshStandardMaterial({
      ...
      emissiveIntensity: 5, // Adjust the intensity for better visibility
      metalness: 0.7, // Adjust the metalness property
      roughness: 0.9 // Adjust the roughness property
  })
...
const guiControls = {
  audioVolume: 1.0,
  rotationSpeed: 0.01,
  audioEnabled: false, // Set the initial state to false to start with audio off
  showOrbitPaths: true,
  stopRotation: false,
};
...
// Function to create circular paths for orbits
...
const uranusOrbitPath = createOrbitPath(257);
const neptuneOrbitPath = createOrbitPath(280);
